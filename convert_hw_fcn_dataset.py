...
def dict_to_tf_example(data, label,name):
    ...
    if height < vgg_16.default_image_size or width < vgg_16.default_image_size:
        print("jump over-->>","name:", name, "height:", height, "width:", width)
        return None
...
def create_tf_record(output_filename, file_pars):
    ...
    file_list = list(file_pars)
    file_pars_length = len(file_list)
    print("file_pars_length:",file_pars_length)
    for idx in range(file_pars_length):
        example = file_list[idx]
        print("idx:",idx)
        if idx % 100 == 0:
            logging.info('On image %d of %d', idx, file_pars_length,"current-->> ", "pic_data:",example[0],"pic_lable:",example[1])
